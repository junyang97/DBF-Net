...
try:
    ...
    from causal_conv1d import causal_conv1d_fn, causal_conv1d_update
    #fused_add_norm_fn = causal_conv1d.fused_add_norm_fn
    # 安全地获取 fused_add_norm_fn，如果不存在则设为 None
...
class Mamba(nn.Module):
    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        if self.use_fast_path and inference_params is None:  # Doesn't support outputting the states
            ...
            #e££££££££££££££££££££££££#
                ...
                if selective_scan_fn is not None:
                    ...
                    # dt -> softplus
                    dt_sp = F.softplus(dt)  # (B*L, dt_rank)
                    # reshape成 (B, dt_rank, L)
                    ...
                    # 将 dt_sp 映射到 d_inner 维度，再截取前 d_state
                    # 保证与 B_feat 的维度匹配
                    ...
                    # 计算增量
                    dA = torch.exp(dt_mapped)  # (B, d_state, L)
                    dB = dt_mapped * B_feat  # (B, d_state, L) ✅ 与 B_feat 维度匹配

                    # 更新状态
                    x_state = x[:, :self.d_state, :]  # 取前 d_state 维度，shape (B, d_state, L)
                    # 更新状态
                    if ssm_state is not None:
                        ...
                        y = x_state * dB  # shape (B, d_state, L)
                    # 保证 y 和 z 维度对齐
                    # z shape: (B, d_inner, L) 或 (B, d_model, L)
                    y_full = z.clone()
                    y_full[:, :self.d_state, :] = y  # 更新前 d_state
                    y = y_full * self.act(z)  # (B, d_model, L) ✅ 维度匹配
        ...
        # 在 forward 函数最后
        if 'out' not in locals():  # 如果任何分支没赋值
            out = hidden_states  # fallback，保证不报错
        return out
